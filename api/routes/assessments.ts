...
assessmentsRoutes.post("/", async (c) => {
  ...
  const body = await c.req.json<{ customerId?: string; title?: string }>();
  ...
  return withClient(async (client) => {
    const own = await client.query(
      `SELECT 1 FROM customer WHERE id = $1 AND owner_oid = $2`,
      [body.customerId, user.oid],
    );
    if (own.rowCount === 0) return c.json({ error: "not_found" }, 404);

    const v = await client.query<{ id: string }>(
      `SELECT id FROM standards_version ORDER BY published_at DESC LIMIT 1`,
    );
    if (v.rowCount === 0) return c.json({ error: "no_standards_seeded" }, 404);
    ...
    const a = await client.query<{ id: string }>(
      `INSERT INTO assessment
       ...
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id`,
      [body.customerId, v.rows[0].id, body.title, user.oid, user.email],
    );
...
assessmentsRoutes.get("/:id", async (c) => {
  ...
  const a = await query<{
    ...
    `SELECT a.id, a.customer_id, a.version_id, a.title, a.status,
  ...
  if (a.rowCount === 0) return c.json({ error: "not_found" }, 404);

  const responses = await query(
    `SELECT r.id, r.sub_criterion_id, r.level, r.not_applicable, r.rationale,
            COALESCE(
              json_agg(
                json_build_object(
                  'id', e.id,
...
assessmentsRoutes.put("/:id/responses/:subCriterionId", async (c) => {
  ...
  const r = await query<{ id: string }>(
    `INSERT INTO response (assessment_id, sub_criterion_id, level, not_applicable, rationale)
     VALUES ($1, $2, $3, $4, $5)
     ...
     DO UPDATE SET level = EXCLUDED.level,
                   ...
                   updated_at = NOW()
     RETURNING id`,
    [id, subId, body.level ?? null, body.notApplicable ?? false, body.rationale ?? null],
  );
...
assessmentsRoutes.post("/:id/responses/:subCriterionId/evidence", async (c) => {
  ...
  const id = c.req.param("id");
  const subId = c.req.param("subCriterionId");
  const body = await c.req.json<{ kind?: string; label?: string; body?: string }>();
  ...
  const r = await query<{ id: string }>(
    `WITH owned AS (
     ...
     RETURNING id`,
    [id, user.oid, subId, body.kind, body.label, body.body],
  );
  if (r.rowCount === 0) {
