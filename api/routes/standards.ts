...
standardsRoutes.get("/current", async (c) => {
  const version = await query<{ id: string; version: string; published_at: string }>(
    `SELECT id, version, published_at
       FROM standards_version
       ORDER BY published_at DESC
       LIMIT 1`,
  );
  if (version.rowCount === 0) return c.json({ error: "no_standards_seeded" }, 404);
  ...
  const subs = await query<{
    ...
    `SELECT sc.id, sc.standard_id, sc.code, sc.text, sc.rationale,
  ...
  const descriptors = await query<{
    ...
    `SELECT ld.id, ld.sub_criterion_id, ld.level, ld.descriptor, ld.evidence_hints
  ...
  const descriptorsBySub = new Map<string, typeof descriptors.rows>();
