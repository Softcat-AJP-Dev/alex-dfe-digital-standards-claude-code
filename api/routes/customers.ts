...
customersRoutes.get("/", async (c) => {
  ...
  const r = await query(
    `SELECT id, name, urn, phase, trust_name, created_at, updated_at
       FROM customer
      WHERE owner_oid = $1
...
customersRoutes.post("/", async (c) => {
  ...
  const r = await query<{ id: string }>(
    `INSERT INTO customer (name, urn, phase, trust_name, owner_oid, owner_email)
     VALUES ($1, $2, $3::"school_phase", $4, $5, $6)
     RETURNING id`,
    [body.name, body.urn ?? null, body.phase, body.trustName ?? null, user.oid, user.email],
  );
...
customersRoutes.get("/:id", async (c) => {
  ...
  const cu = await query(
    `SELECT id, name, urn, phase, trust_name, created_at, updated_at
       FROM customer
      WHERE id = $1 AND owner_oid = $2`,
    [id, user.oid],
  );
  if (cu.rowCount === 0) return c.json({ error: "not_found" }, 404);
